...
Second, we report performance of open-source weakly-supervised models such as CLIP~\citep{radford2021learning}, OpenCLIP~\citep{ilharco_gabriel_2021_5143773, cherti2023reproducible}, and SWAG~\citep{singh2022revisiting}.
...
\midrule
    ...
    CLIP        & ViT-L/14$_{\text{336}}$ & WIT-400M & \checkmark  && 80.5 && 85.3 & 88.8 & 75.8 \\
...
\multicolumn{10}{@{}c@{}}{\textbf{Self-supervised }}\\
  ...
  \caption{
    ...
    We compare across any possible architectures (Arch.), at resolution $224 \times 224$ unless stated otherwise.
...
In Table~\ref{tab:ft-inet1k-alone}, we show that the Top-1 accuracy on the validation set of ImageNet-1k improves by more than $+2\%$ when the backbone is finetuned.
...
Nonetheless, our best finetuned performance ($88.9\%$) is only a couple of percent below ($-2.2\%$) the absolute state of the arts ($91.1\%$), obtained by~\citet{chen2023symbolic}.
...
\begin{table}[ht]
  ...
  \caption{
    ...
    We use the pipeline of~\cite{touvron2022deit} to finetune our encoders on ImageNet-1k at resolutions $224\times224$ or $448\times448$. We compare with the accuracy obtained with linear probing and observe only modest improvements with fine-tuning: this suggests that \OURS{} features already perform well out-of-the-box.
  }
...
When comparing with state-of-the-art SSL methods, our models shows drastically better robustness ($+29.6\%$ on A\rone{~\citep{hendrycks2021natural}}, $+22.1\%$ on R\rone{~\citep{hendrycks2021many}} and $+23.0\%$ on Sketch\rone{~\citep{wang2019learning}} compared to iBOT).
...
\begin{table}[t]
  ...
  \begin{tabu}{@{} lll c cccc @{}}
    ...
    \midrule
    OpenCLIP    & ViT-G/14 & LAION-2B      && 63.8 &\bf 87.8 & 45.3 & \bf 66.4 \\
    \midrule
...
\cmidrule{4-6}\cmidrule{8-10}
    ...
    \midrule
    \multirow{4}{*}{\OURS}  & ViT-S/14 && 69.0 & 74.2 & 62.9 && 67.8 & 87.0  & 33.1 \\ 
...
Interestingly, our evaluation using \textbf{+ms} is on par with fully finetuning MAE with an Upernet decoder ($53.0$ versus $53.6$ mIoU).
...
\rone{
\paragraph{Frozen backbone in a SOTA pipeline.} 
...
With this setup, we reach $60.2$ mIoU on ADE20k, close to the competitive state of the art, standing at 62.9 mIoU~\citep{wang2022internimage}.
Although our setup for this experiment doesn't makes use of the optimisations described in Sec.~\ref{sec:optim}, the segmentation training in this experiment took 28 hours on 16 V100 GPUs.
}
...
\textbf{lin. 4}: we use the same protocol that we use with one layer, but concatenate the tokens from layers $l=\{3,6,9,12\}$ for ViT-S/B, $l=\{5,12,18,24\}$ for ViT-L, and $l=\{10, 20, 30, 40\}$ for ViT-g.
...
\begin{table}[t]
  ...
  \begin{tabu}{@{}ll@{} c ccc c ccc c ccc@{}}
    \toprule
    & && \multicolumn{3}{c}{NYUd} && \multicolumn{3}{c}{KITTI} && \multicolumn{3}{c}{NYUd $\rightarrow$ SUN RGB-D} \\
    & && 
